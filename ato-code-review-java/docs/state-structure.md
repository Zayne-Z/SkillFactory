...
Phase 1 须让分支 + 上表各项选项（含 `max_lines_per_batch`）都有值；可分多轮收集，用户跳过时使用默认值。复述后设 `user_confirmed: true` 再进入 `diff_analysis`。断点续跑时子 agent 通过主编排 Agent 传入的 `SEVERITY_MODE` 等变量读取此配置。
