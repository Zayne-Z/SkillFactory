 ...
 *   {output-dir}/batch-001.patch — 该批所有文件的合并 diff（单次或分块 git 调用）
 */
...
function allocateChangeChunk(remainingDeletes, remainingAdds, chunksLeft, maxLines) {
  ...
  const maxDeletes = Math.min(remainingDeletes, takeTotal, takeTotal - Math.max(0, remainingAdds - futureCapacity));
...
function hunkUnits(hunk, maxLines) {
  ...
  while (index < hunk.entries.length) {
    if (hunk.entries[index].type === ' ') { index++; continue; }
    ...
    for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
      const allocation = allocateChangeChunk(deletes.length - deleteOffset, adds.length - addOffset, chunkCount - chunkIndex, maxLines);
      ...
      const before = chunkIndex === 0 ? hunk.entries.slice(Math.max(0, runStart - 3), runStart).filter((entry) => entry.type === ' ') : [];
      const after = chunkIndex === chunkCount - 1 ? hunk.entries.slice(runEnd, runEnd + 3).filter((entry) => entry.type === ' ') : [];
      ...
      units.push({
        suffix: hunk.suffix,
        oldStart: firstContext?.oldAnchor ?? firstDelete?.oldAnchor ?? firstAdd?.oldAnchor ?? 1,
        newStart: firstContext?.newAnchor ?? firstAdd?.newAnchor ?? firstDelete?.newAnchor ?? 1,
        entries,
...
function splitOversizedPatch(text, maxLines) {
  ...
  for (const line of lines) {
    ...
    if (match) {
      hunk = { oldLine: Number.parseInt(match[1], 10), newLine: Number.parseInt(match[3], 10), suffix: match[5] || '', entries: [] };
      hunks.push(hunk);
      continue;
    }
    if (!hunk) { prefix.push(line); continue; }
    const type = line[0];
  ...
  for (const unit of units) {
    if (current.length && currentLines + unit.changedLines > maxLines) { grouped.push(current); current = []; currentLines = 0; }
    current.push(unit);
  ...
  const parts = grouped.map((group, partIndex) => {
    ...
    ordinal = end + 1;
    return { text: `${out.join('\n').replace(/\n+$/, '')}\n`, changedLines: partLines, lineRanges: mergeLineRanges(ownedLines), slice: { index: partIndex + 1, total: grouped.length, start_changed_line: start, end_changed_line: end } };
  });
  return parts;
}
...
function main() {
  ...
  for (const batch of batches) {
    ...
    const selectedSlices = segmented ? slices.filter((slice) => slice.slice.index === Number(batch.diff_slice.index)) : slices;
    if (segmented && selectedSlices.length !== 1) throw new Error(`无法按原边界重建 ${batch.id || batch.batch_id}，请重新执行分批`);
    if (selectedSlices.length) {
      for (const slice of selectedSlices) {
        const file = { ...(batch.files[0] || {}), changed_lines: String(slice.changedLines), line_ranges: slice.lineRanges, diff_slice: slice.slice };
        prepared.push({ patch: slice.text, preserveId: segmented || slice.slice.index === 1 ? (batch.id || batch.batch_id) : '', batch: { ...batch, description: `${batch.description || paths[0]} [${slice.slice.index}/${slice.slice.total}]`, files: [file], total_lines: String(slice.changedLines), oversized: false, segmented: true, diff_slice: slice.slice }, lineRanges: slice.lineRanges });
      }
    } else {
  ...
  inventory.total_batches = String(expandedBatches.length);
